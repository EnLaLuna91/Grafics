...
void main()
{
    ...
    // Luz Puntual
    if (luz[0].on == 1){
        vec3 L = (luz[0].coordenadas.xyz - vPosition.xyz); // de la luz respecto al objeto
        phong1 = calculateAtenuation(0) * calculatePhong(0, L);
    }

    // Luz Direccional
    if (luz[1].on == 1){
        vec3 L = (luz[1].direccion.xyz - vPosition.xyz); // de la luz respecto al objeto
        phong2 = 1.0f * calculatePhong(1, L);
    }

    // Luz SpotLight
    if (luz[2].on == 1){
        vec3 L = (luz[2].direccion.xyz - vPosition.xyz); // de la luz respecto al objeto
...
//    vec3 L = (luz[0].coordenadas.xyz - vPosition.xyz); // Posicion luz
